...
/*   Updated: 2022/03/22 10:20:34 by bkeskint         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**arguments(char const *s, char c)
{
	...
	s_arr = (char **)ft_calloc((arguments_counter(s, c) + 1), sizeof(char *));
	if (!s_arr)
		return (s_arr);
	i = 0;
	s_arr2 = (char *)s;
	...
	while (i < arguments_counter(s, c))
	{
		...
		s_arr[i] = (char *)ft_calloc((len2 + 1), sizeof(char));
		if (!s_arr[i])
			return (NULL);
...
void	route(t_parameters *params)
{
	...
	ft_bzero(params->line, ft_strlen(params->line));
	free (params->line);
}
